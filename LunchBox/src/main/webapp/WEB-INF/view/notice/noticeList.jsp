...
<c:if test="${noticeArticlePage.hasNoArticles()}">
	<tr>
		<td colspan="4">게시글이 없습니다.</td>
	</tr>
</c:if>
<c:forEach var="article" items="${noticeArticlePage.content}">
	<tr>
		<td>${article.articleNo}</td>
		<td>
		<a href="/notice/view?articleNo=${article.articleNo}&pageNo=${noticeArticlePage.currentPage}">
...
<div class = "pageinfo">
<c:if test="${noticeArticlePage.hasArticles()}">
	<ul class="pager">
		<li>
		<c:if test="${noticeArticlePage.startPage > 5}">
		<a href="/notice/list?pageNo=${noticeArticlePage.startPage - 5}">[이전]</a>
		</c:if>
		</li>
		<li>
		<c:forEach var="pNo"  begin="${noticeArticlePage.startPage}" end="${noticeArticlePage.endPage}">
		<a class=no href="/notice/list?pageNo=${pNo}">${pNo}</a>
		</c:forEach>
		</li>
		<li>
		<c:if test="${noticeArticlePage.endPage < noticeArticlePage.totalPages}">
			<a href="/notice/list?pageNo=${noticeArticlePage.startPage + 5}">[다음]</a>
			</c:if>
			</li>
	</ul>
</c:if>
...
<c:if test="${memberSession.memTypeNo==11}">
	<p class="btn-write"><a href="/notice/write" id="write">글 등록</a><p>
</c:if>
</div>

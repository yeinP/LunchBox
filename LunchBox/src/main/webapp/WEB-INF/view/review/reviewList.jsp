...
<c:if test="${reviewArticlePage.hasNoArticles()}">
	<tr>
		<td colspan="4">게시글이 없습니다.</td>
	</tr>
</c:if>
<c:forEach var="article" items="${reviewArticlePage.content}">
	<tr>
		<td>${article.articleNo}</td>
		<td>
		<a href="${pageContext.request.contextPath}/board/detail?articleNo=${article.articleNo}&showNo=${st.index+1}&pageNo=${articlePage.currentPage}">
		<c:out value="${article.articleTitle}"/>
		</a>
		</td>
		<td>${article.memId}</td>
		<td><tf:formatDateTime value="${article.articleRegDate}" pattern="yyyy-MM-dd "/></td>
		<td>${article.articleHits}</td>
	</tr>
</c:forEach>
</table>
<div class = "pageinfo">
<c:if test="${reviewArticlePage.hasArticles()}">
	<ul class="pager">
		<li>
		<c:if test="${reviewArticlePage.startPage > 5}">
		<a href="/review/list?pageNo=${reviewArticlePage.startPage - 5}">[이전]</a>
		</c:if>
		</li>
		<li>
		<c:forEach var="pNo"  begin="${reviewArticlePage.startPage}" end="${reviewArticlePage.endPage}">
		<a class=no href="/review/list?pageNo=${pNo}">${pNo}</a>
		</c:forEach>
		</li>
		<li>
		<c:if test="${reviewArticlePage.endPage < reviewArticlePage.totalPages}">
			<a href="/review/list?pageNo=${reviewArticlePage.startPage + 5}">[다음]</a>
			</c:if>
			</li>
	</ul>
</c:if>
...
<div class="write">
<c:if test="${memberSession.memTypeNo==99}">
	<p class="btn-write"><a href="/review/write" id="write">글 등록</a><p>
</c:if>
</div>

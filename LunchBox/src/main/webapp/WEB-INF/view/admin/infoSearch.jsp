...
<div class="searchWrap" style="display:flex;">
			<form action = "/admin/infoSearch" method="get" style="margin:auto;">
				<input type="date" id = "date1" name = "startDate" value = "${startDate}"> ~ <input type="date" id = "date2" name = "endDate" value="${endDate}">
	        	<input type="text" value="${keyword}" name = "keyword" id="keyword">
	        	<input type="submit" value="검색">
	        </form>
	    </div>
	<div class="membersinfoBox" style="display:none; margin-top: 20px;"  id="orderInfoDiv">
	
		
			<%-- <form:form  modelAttribute="cmd">
			<p>
			<label style="padding-right: 1em;"><span style="padding-right:0.5em; font-weight: bold">From</span><form:input path="from" placeholder="ex) 20230101"/></label>
			~
			<label style="padding-left: 1em;"> <span style="padding-right:0.5em; font-weight: bold">To</span><form:input path="to" placeholder="ex) 20231231"/></label>
			<input type="submit" value="조회">
			</p>
			</form:form> --%>
		</div>
		<div class="tableall" style="margin-top: 20px;">
			<c:if test="${!empty orderList}">
					<table style="text-align: center; border:1px solid gray; width:700px;">
					<colgroup>
						<col width="20%;">
								<col width="20%">
								<col width="40%">
					 ...
					 <tr>
						<th style="padding: 10px; margin: 20px; border-bottom:1px solid gray; background-color: white;" >아이디</th>
								 <th style="padding: 10px; margin: 20px; border-bottom:1px solid gray; background-color: white;">주문일</th>
								 <th style="padding: 10px; margin: 20px; border-bottom:1px solid gray; background-color: white;">주문 상품</th>
								 <th style="padding: 10px; margin: 20px; border-bottom:1px solid gray; background-color: white;">결제 금액</th>
					 </tr>
		 			<c:forEach var = "mem" items="${orderList}">
			 			<tr>
							 
							<td style="border-bottom:1px solid gray; ">${mem.memId}</td>
									 <td style="border-bottom:1px solid gray; "><tf:formatDateTime value="${mem.orderDate}" pattern="yyyy.MM.dd"/></td>
									 <td style="border-bottom:1px solid gray; ">${mem.productName}</td>
									 <td style="border-bottom:1px solid gray; ">${mem.productPrice}</td>
						</tr>
		 			</c:forEach>

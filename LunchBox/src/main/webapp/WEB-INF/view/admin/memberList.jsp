...
<col width="*%">
 ...
 <c:if test="${memberList.hasNoArticles()}">
	<tr>
		<td colspan="6">회원이 없습니다.</td>
	</tr>
</c:if>
 <c:forEach var="mem" items="${memberList.content}">
 <tr>
 <td>
    <a href="/admin/memberOrderInfo/${mem.memId}" target="_blank" onclick="openPopup(this.href, 'memberOrderInfo', 1500, 1500); return false;">${mem.memId}</a>
</td>
 <td>${mem.memEmail}</td>
 <td>${mem.memName}</td>
 <td><tf:formatDateTime value="${mem.memRegDate}" pattern="yyyy.MM.dd"/></td>
 <td>${mem.memPhone}</td>
...
<c:if test="${memberList.hasArticles()}">
	<ul class="pager">
		...
		<a href="/admin/memberList?pageNo=${memberList.startPage - 5}">[이전]</a>
		</c:if>
		</li>
		<li>
		<c:forEach var="pNo"  begin="${memberList.startPage}" end="${memberList.endPage}">
		<a class=no href="/admin/memberList?pageNo=${pNo}">${pNo}</a>
		</c:forEach>
		</li>
		<li>
		<c:if test="${memberList.endPage < memberList.totalPages}">
			<a href="/admin/memberList?pageNo=${memberList.startPage + 5}">[다음]</a>
			</c:if>
			</li>
	</ul>
</c:if>
